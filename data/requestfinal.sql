...
SELECT id AS idpost, title AS titlepost, LEFT(content,255) AS contentshort, datecreate FROM post WHERE visible=1 ORDER BY datecreate DESC;
...
SELECT id AS idpost, title AS titlepost, LEFT(content,255) AS contentshort, datecreate FROM post WHERE !isnull(user_id) ORDER BY datecreate DESC;
...
SELECT id AS idpost, title AS titlepost, LEFT(content,255) AS contentshort, datecreate FROM post WHERE !isnull(user_id) && visible = 1 ORDER BY datecreate DESC;
...
SELECT p.id, p.title, p.datecreate
FROM post p
	LEFT JOIN category_has_post h
    ON p.id = h.post_id
    WHERE isnull(h.post_id)
ORDER BY p.datecreate ASC;
